...
{
	if(weapon == -1 || GetEntProp(weapon, Prop_Data, "m_iHammerID") != 2233)
		return;
...
int DropBlocker(int client, int weapon)
{
	...
	if(wep_Secondary == weapon && GetEntityClassname(weapon, classname, sizeof(classname)) && strcmp(classname, "weapon_pistol") == 0
		&& !GetEntProp(weapon, Prop_Send, "m_isDualWielding"))
	{
		return false;
	}
...
void DropWeapon(int client, int weapon)
{
	...
	if(isSecondary)
	{
		...
		if(pistol != -1)
		{
			...
			SetEntProp(pistol, Prop_Data, "m_iHammerID", 2233);
		}
	}
