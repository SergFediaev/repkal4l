...
public void OnPluginStart()
{
	...
	// Sefo
	// RegConsoleCmd("sm_info", printInfo);
	RegConsoleCmd("sm_server", printServer);
	RegConsoleCmd("sm_donate", printDonate);
	RegConsoleCmd("sm_commands", printCommands);
	RegConsoleCmd("sm_help", printHelp);
	RegConsoleCmd("sm_guide", printHelp);
	RegConsoleCmd("sm_manual", printHelp);
	RegConsoleCmd("sm_rtfm", printHelp);
}

// Sefo
public Action printServer(int client, int arguments)
{
	CPrintToChatAll("%t", "Server");
	return Plugin_Handled;
}

// Sefo
public Action printDonate(int client, int arguments)
{
	CPrintToChatAll("%t", "Donate");
	return Plugin_Handled;
}

// Sefo
public Action printCommands(int client, int arguments)
{
	CPrintToChatAll("%t", "Commands");
	return Plugin_Handled;
}

// Sefo
public Action printHelp(int client, int arguments)
{
	CPrintToChatAll("%t", "Help");
	return Plugin_Handled;
}
...
{
	for (int i = 1; i <= MaxClients; i++)
		if (IsClientInGame(i) && !IsFakeClient(i) && !g_bLeave[i])
		{
			SteamWorks_GetUserGroupStatusAuthID(g_iAuthID[i], g_iGroupId);
		}
		...
		if (IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			else
			{
				switch (g_iMessageId)
				{
					...
					{
						g_iMessageId = 2;
						CPrintToChat(i, "%t", "Server");
						break;
					}
					case 1:
					{
						g_iMessageId = 2;
						CPrintToChat(i, "%t", "Donate");
						break;
					}
					case 2:
					{
						g_iMessageId = 0;
						CPrintToChat(i, "%t", "Join");
						break;
					}
				}
			}
