...
public void OnPluginStart()
{
    ...
    C_enable = CreateConVar(PLUGIN_PREFIX ... "_enable", "3", "1 = enable defib health settings, 2 = enable rescue health settings. add numbers together", _, true, 0.0, true, 3.0);
    C_enable.AddChangeHook(convar_changed);

    C_defib_health_true = CreateConVar(PLUGIN_PREFIX ... "_defib_health_true", "30", "how many main health will get after defibrillator used", _, true, 1.0);
    C_defib_health_true.AddChangeHook(convar_changed);
    C_defib_health_buffer = CreateConVar(PLUGIN_PREFIX ... "_defib_health_buffer", "0.0", "how many health buffer will get after defibrillator used", _, true, 0.0);
    C_defib_health_buffer.AddChangeHook(convar_changed);
    C_defib_dying = CreateConVar(PLUGIN_PREFIX ... "_defib_dying", "0", "1 = enable, 0 = disable. set \"m_isGoingToDie\" prop after defibrillator used?");
    C_defib_dying.AddChangeHook(convar_changed);
    C_defib_revive_count = CreateConVar(PLUGIN_PREFIX ... "_defib_revive_count", "0", "how many revive count will be set after defibrillator used?", _, true, 0.0);
    C_defib_revive_count.AddChangeHook(convar_changed);

    C_rescue_health_true = CreateConVar(PLUGIN_PREFIX ... "_rescue_health_true", "30", "how many main health will get after rescued", _, true, 1.0);
    C_rescue_health_true.AddChangeHook(convar_changed);
    C_rescue_health_buffer = CreateConVar(PLUGIN_PREFIX ... "_rescue_health_buffer", "0.0", "how many health buffer will get after rescued", _, true, 0.0);
    C_rescue_health_buffer.AddChangeHook(convar_changed);
    C_rescue_dying = CreateConVar(PLUGIN_PREFIX ... "_rescue_dying", "0", "1 = enable, 0 = disable. set \"m_isGoingToDie\" prop after rescued?");
    C_rescue_dying.AddChangeHook(convar_changed);
    C_rescue_revive_count = CreateConVar(PLUGIN_PREFIX ... "_rescue_revive_count", "0", "how many revive count will be set after rescued?", _, true, 0.0);
