...
ConVar cProgressTargets;
int	   iProgressTargets;
ConVar cAllowMedicines;
int	   iAllowMedicines;
ConVar cActions;
int	   iActions;
ConVar cPillsBuff;
float  flPillsBuff;
ConVar cAdrenalineBuff;
float  flAdrenalineBuff;
ConVar cHealthMax;
float  flHealthMax;
ConVar cFirstAidCap;
ConVar cOverflowTurn;
float  flOverflowTurn;
ConVar cAdrenalineDuration;
float  flAdrenalineDuration;
ConVar cUseDuration;
float  flUseDuration;
ConVar cReward;
float  flReward;
...
{
	if (iProgressTargets & Provider)
		SetupProgressBar(provider, flUseDuration, 1, receiver, provider, true);	   // Sefo
...
public Action
	OnFeedingAnimation(int client, int &sequence)
{
	if (iFeedingTarget[client])
	{
...
void SetupProgressBar(int client, float time = 0.0, int action = 0, int entity_target = -1, int entity_owner = -1, bool isProvider = false)	   // Sefo
{
	if (!IsSurvivor(client))
		return;
	...
	if (bIsLeft4Dead2)
	{
		if (isProvider) SetEntPropEnt(client, Prop_Send, "m_useActionTarget", entity_target);	 // Sefo

		if (isProvider) SetEntPropEnt(client, Prop_Send, "m_useActionOwner", entity_owner);	   // Sefo
