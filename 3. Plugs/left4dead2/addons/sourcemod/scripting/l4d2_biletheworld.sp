// AtomicStryker, foxhound27, HarryPotter, Sefo Notasi (Notasi.ru) & firegaming @ 2010-2022
...
public Plugin myinfo = 
{
	...
	author = "AtomicStryker, foxhound27, HarryPotter, Sefo Notasi (Notasi.ru) & firegaming",
...
public void OnPluginStart()
{
	...
	cvar_BoomerDeath = CreateConVar("l4d2_bile_the_world_boomer_death", "1", "1 - CI, SI, Tank, Witch\n2 - SI\n3 - Witch, Tank\n4 - SI, Tank, Witch", FCVAR_NOTIFY, true, 0.0, true, 4.0); 
...
void GetCvars()
{
	g_iBoomerDeath = cvar_BoomerDeath.IntValue;
...
{		
	...
	if (BoomerDeath)
	{
		switch(g_iBoomerDeath)
		{
			case 1:
			{
				VomitJarSi(pos, client);
				VomitJarTank(pos, client);
				VomitJarWitch(pos, client);
				VomitJarCI(pos, client);
			}
...
void VomitJarSi(const float pos[3], int client)
{
	float targetpos[3];
	for(int target = 1; target <= MaxClients; ++target)
	{
		if(!IsClientInGame(target) || GetClientTeam(target) != L4D2Team_Infected || !IsPlayerAlive(target) || 
			GetEntProp(target, Prop_Send, ENTPROP_IS_GHOST) || GetEntProp(target, Prop_Send, "m_zombieClass") == 8)
		{
			continue;
		}
		...
		L4D2_CTerrorPlayer_OnHitByVomitJar(target, client);
	}
}

void VomitJarTank(const float pos[3], int client)
{
	float targetpos[3];
	for(int target = 1; target <= MaxClients; ++target)
	{
...
void VomitJarCI(const float pos[3], int client)
{
	int infected = MaxClients+1;
	float targetpos[3];
	while((infected = FindEntityByClassname(infected, CLASS_ZOMBIE)) != -1)
	{
		SetClientEyePosition(infected, targetpos);
		if (GetVectorDistance(pos, targetpos) > g_fBoomerDeath_Radius || !IsVisibleTo(pos, targetpos))
			continue;
		
		L4D2_Infected_OnHitByVomitJar(infected, client);
	}
}

void VomitJarWitch(const float pos[3], int client)
{
	int witch = MaxClients+1;
	float targetpos[3];
	while((witch = FindEntityByClassname(witch, CLASS_WITCH)) != -1)
	{
		SetClientEyePosition(witch, targetpos);
		if (GetVectorDistance(pos, targetpos) > g_fBoomerDeath_Radius || !IsVisibleTo(pos, targetpos))
			continue;
...
bool IsVisibleTo(const float position[3], const float targetposition[3])
{
